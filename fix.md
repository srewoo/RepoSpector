...
| `SAVE_SETTINGS` rejected when popup runs as floating-panel iframe | Reported as "unable to save settings" with `[messageRouter] Rejected SAVE_SETTINGS: Handler does not accept content-script messages`. Root cause: `Settings.jsx` bypasses the `useExtension()` hook and calls `chrome.runtime.sendMessage` directly. When the popup is loaded as a `chrome-extension://...?mode=iframe` iframe inside a host page (the floating panel), Chrome populates `sender.tab` even though the iframe is at our own origin — so the previous `isFromPopup` bypass didn't apply. Fix: trust the message whenever `sender.url` starts with `chrome-extension://<our-id>/`. Covers popup, options, and any embedded iframe without depending on every caller remembering a flag. The id check still fires first so forged URLs can't escalate. 2 more router tests. |
